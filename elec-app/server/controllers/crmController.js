...
// Engineers may manage commercial percentages without learning any monetary
// values. Keep percentage fields visible and redact only currency/amount data.
const ENGINEER_PRICE_FIELDS = ['base_price_usd','base_price_euro','custom_price_usd','custom_price_euro','price_usd','price_euro','markupP_amt','discount_amt','totalpriceT','manpower_amt','markupM_amt','totalfinalProduct','cost','cr_amount','total_price','total_cost','total_final','total_product','total_manpower'];
function redactEngineerPrices(rows) {
  for (const row of rows || []) for (const field of ENGINEER_PRICE_FIELDS) if (field in row) row[field] = null;
}
...
async function checkProjectAccess(req, res, projectId) {
  ...
  // Keep API authorization aligned with ROLE_PERMISSIONS. Accounting may read
  // projects/CRM for invoicing; secretary has no projects permission and must
  // not gain access merely by calling the API directly.
  if (!['owner', 'head_engineer', 'accounting', 'engineer'].includes(user.role)) {
    res.status(403).json({ error: 'Access denied - your role cannot access projects' });
    return false;
  }
  return true;
}
...
async function createPanel(req, res, next) {
  try {
    ...
    const [result] = await db.execute(
      'INSERT INTO project_crm_panels(project_id,panel_number,panel_name,quantity,markupP,markupM,manpower_pct) VALUES(?,?,?,?,?,?,?)',
      [req.params.projectId, panel_number, panel_name||null, quantity, markupP||0, markupM||0, manpower_pct||0]
    );
...
async function updatePanel(req, res, next) {
  try {
    ...
    if (Number(panel.quantity || 1) !== quantity) {
      const [affected] = await db.execute(`SELECT product_id,SUM(qty) item_qty FROM panel_crm_items
        WHERE division_id IN (SELECT id FROM panel_divisions WHERE panel_id=?) AND product_id IS NOT NULL GROUP BY product_id`, [req.params.panelId]);
      for (const item of affected) {
        const oldQty = Number(item.item_qty) * Number(panel.quantity || 1);
        const newQty = Number(item.item_qty) * quantity;
        await db.execute(`INSERT INTO reservation_history(product_id,project_id,panel_id,old_qty,new_qty,change_qty,reason,changed_by)
          VALUES(?,?,?,?,?,?,?,?)`, [item.product_id, req.params.projectId, req.params.panelId, oldQty, newQty, newQty-oldQty, 'panel_quantity_changed', req.worker.id]);
      }
    }
    ...
    const [rows] = await db.execute(
      `SELECT pcp.*, w.name as updated_by_name
       ...
       WHERE pcp.id=? AND pcp.project_id=?`,
      [req.params.panelId, req.params.projectId]
    );
    if (req.worker.role === 'engineer') redactEngineerPrices(rows);
    res.json(rows[0]);
...
async function deletePanel(req, res, next) {
  try {
    ...
    const [[deletedPanel]] = await db.execute(
      'SELECT panel_name, panel_number, project_id FROM project_crm_panels WHERE id=? AND project_id=?',
      [req.params.panelId, req.params.projectId]
    );
...
async function getDivisions(req, res, next) {
  try {
    ...
    const [divisions] = await db.execute(
      `SELECT d.*,dt.color division_color,
        (SELECT COUNT(*) FROM panel_crm_items i WHERE i.division_id=d.id) as item_count
       FROM panel_divisions d LEFT JOIN division_types dt ON dt.name=d.division_type WHERE d.panel_id=? ORDER BY d.id`,
      [req.params.panelId]
    );
    if (req.worker.role === 'engineer') {
      redactEngineerPrices(divisions);
    }
    res.json(divisions);
...
async function createDivision(req, res, next) {
  try {
    ...
    const [[validType]] = await db.execute('SELECT id FROM division_types WHERE name=? AND is_active=1',[division_type]);
    if (!validType) return res.status(400).json({ error: 'Choose an active division type from Division Types settings' });
    ...
    const [rows] = await db.execute('SELECT d.*,dt.color division_color FROM panel_divisions d LEFT JOIN division_types dt ON dt.name=d.division_type WHERE d.id=?', [result.insertId]);
    if (req.worker.role === 'engineer') redactEngineerPrices(rows);
...
async function getManualProducts(req, res, next) {
  try {
    ...
    if (req.worker.role === 'engineer') redactEngineerPrices(products);
    res.json(products);
...
async function createManualProduct(req, res, next) {
  try {
    ...
    const [result] = await db.execute(
      ...
      [req.params.projectId, name, description||null, req.worker.role === 'engineer' ? null : (price_euro||null), req.worker.role === 'engineer' ? null : (price_usd||null), brand||null]
    );
...
async function checkDivisionAccess(req, res, divisionId, expectedProjectId, expectedPanelId) {
  const [[div]] = await db.execute(`
    SELECT d.panel_id, pcp.project_id FROM panel_divisions d
    JOIN project_crm_panels pcp ON d.panel_id = pcp.id
    WHERE d.id = ?`, [divisionId]);
  if (!div) { res.status(404).json({ error: 'Division not found' }); return false; }
  if ((expectedProjectId !== undefined && String(div.project_id) !== String(expectedProjectId)) ||
      (expectedPanelId !== undefined && String(div.panel_id) !== String(expectedPanelId))) {
    res.status(404).json({ error: 'Division not found in this project panel' });
    return false;
  }
...
async function getCrmItems(req, res, next) {
  try {
    ...
    if (req.worker.role === 'engineer') redactEngineerPrices(items);
    res.json(items);
...
async function createCrmItem(req, res, next) {
  try {
    ...
    const parsedQty = qty === undefined ? 1 : Number(qty);
    ...
    const rate = projRows.length ? parseFloat(projRows[0].exchange_rate_eur_usd) || 1.18 : 1.18;

    // Auto-convert prices: fill in whichever is missing
    let eur = base_price_euro;
    let usd = base_price_usd;
    // Price-hidden roles select a product by id without receiving its prices.
    // Resolve them server-side so the CRM calculation remains correct.
    if (!usd && !eur && product_id) {
      const [[productPrice]] = await db.execute('SELECT price_usd,price_euro FROM products WHERE id=?', [product_id]);
      usd = productPrice?.price_usd || null;
      eur = productPrice?.price_euro || null;
    }
    ...
    const pricing = calcItemPricing({
      ...
      manpower_pct: manpower_pct||0, markupM_pct: markupM_pct||0, qty: parsedQty
    });
    ...
    const [result] = await db.execute(
      ...
      [
        ...
        parsedQty, usd||null, eur||null, markupP_pct||0, disc||0, manpower_pct||0, markupM_pct||0,
    ...
    // Return the same enriched shape as getCrmItems. The client inserts this
    // response into its local list, so returning only the raw row made the
    // product name/reference appear blank until a full refresh.
    const [rows] = await db.execute(
      `SELECT i.*, p.reference, p.description as product_desc, b.name as brand_name,
              p.price_euro, p.price_usd
       FROM panel_crm_items i
    ...
    if (rows[0]?.is_manual && rows[0].manual_product_id) {
      const [[manualProduct]] = await db.execute(
        'SELECT name,description,brand,price_euro,price_usd FROM panel_manual_products WHERE id=?',
        [rows[0].manual_product_id]
      );
      if (manualProduct) {
        rows[0].custom_name = manualProduct.name;
        rows[0].custom_desc = manualProduct.description;
        rows[0].custom_brand = manualProduct.brand;
        rows[0].custom_price_euro = manualProduct.price_euro;
        rows[0].custom_price_usd = manualProduct.price_usd;
      }
    }
    if (req.worker.role === 'engineer') redactEngineerPrices(rows);
...
async function updateCrmItem(req, res, next) {
  try {
    ...
    if (req.worker.role === 'engineer') {
      const forbiddenMoneyFields = [base_price_usd, base_price_euro, cost, cr_amount, custom_price_euro, custom_price_usd];
      if (forbiddenMoneyFields.some(value => value !== undefined)) {
        return res.status(403).json({ error: 'Engineers cannot view or change monetary prices. Markup, discount, manpower and quantity are allowed.' });
      }
    }
  ...
  try {
    ...
    if (panels.length) {
      ...
      const [allDivisions] = await db.query(
        `SELECT d.*,dt.color division_color, COUNT(i.id) as item_count
         FROM panel_divisions d
         LEFT JOIN division_types dt ON dt.name=d.division_type
    ...
    if (req.worker.role === 'engineer') {
      const priceFields = ['base_price_usd','base_price_euro','custom_price_usd','custom_price_euro','price_usd','price_euro','markupP_amt','discount_amt','totalpriceT','manpower_amt','markupM_amt','totalfinalProduct','cost','cr_amount'];
      for (const panel of panels) {
        panel.total_price = null;
        for (const div of panel.divisions || []) {
          for (const item of div.items || []) for (const field of priceFields) item[field] = null;
        }
      }
      for (const field of ['total_cost','total_price','project_discount_amount','total_vat','total_with_vat','vat_pct','project_discount_pct','exchange_rate_eur_usd']) project[0][field] = null;
      for (const item of manualProducts) for (const field of ['price_usd','price_euro']) item[field] = null;
    }
...
async function copyPanelFromProject(req, res, next) {
  ...
  try {
    ...
    const [maxNum] = await conn.execute('SELECT COALESCE(MAX(panel_number), 0) + 1 AS next_num FROM project_crm_panels WHERE project_id=? FOR UPDATE', [targetProjectId]);
    const nextPanelNum = maxNum[0].next_num;

    // 3. Create new panel
    const [panelResult] = await conn.execute(
      'INSERT INTO project_crm_panels(project_id,panel_number,panel_name,quantity,markupP,markupM,manpower_pct,note,show_note_in_client_pdf,updated_by) VALUES(?,?,?,?,?,?,?,?,?,?)',
      [targetProjectId, nextPanelNum, sourcePanel.panel_name || `Panel #${nextPanelNum}`, sourcePanel.quantity || 1, sourcePanel.markupP, sourcePanel.markupM, sourcePanel.manpower_pct, sourcePanel.note, sourcePanel.show_note_in_client_pdf, currentUserId]
    );
    const newPanelId = panelResult.insertId;

    // 4. Fetch source divisions
    const [sourceDivs] = await conn.execute('SELECT * FROM panel_divisions WHERE panel_id=?', [sourcePanelId]);
    ...
    for (const sd of sourceDivs) {
      ...
      const [sourceItems] = await conn.execute('SELECT * FROM panel_crm_items WHERE division_id=?', [sd.id]);
      ...
      for (const si of sourceItems) {
        ...
        if (si.is_manual && si.manual_product_id) {
          if (!manualProductMap[si.manual_product_id]) {
            const [mp] = await conn.execute('SELECT * FROM panel_manual_products WHERE id=?', [si.manual_product_id]);
            if (mp.length) {
    ...
    console.error('[CRM] ❌ copyPanelFromProject:', err.message);
    next(err);
  } finally {
  ...
  try {
    ...
    for (const itemId of item_ids) {
      const [[item]] = await db.execute(
        `SELECT pci.* FROM panel_crm_items pci
         JOIN panel_divisions pd ON pd.id=pci.division_id
         JOIN project_crm_panels pcp ON pcp.id=pd.panel_id
         WHERE pci.id=? AND pcp.project_id=?`,
        [itemId, projectId]
      );
      if (!item) continue;
      ...
      if (changes.cr_amount !== undefined) { fields.push('cr_amount=?'); params.push(Math.max(0, parseFloat(changes.cr_amount) || 0)); }
    ...
    const [affected] = await db.execute(
      `SELECT DISTINCT pci.division_id, pd.panel_id
       ...
       JOIN project_crm_panels pcp ON pcp.id = pd.panel_id
       WHERE pci.id IN (${item_ids.map(() => '?').join(',')}) AND pcp.project_id=?`,
      [...item_ids, projectId]
    );
