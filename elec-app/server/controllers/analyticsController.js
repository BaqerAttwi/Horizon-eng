...
export async function getEngineerStats(req, res, next) {
  try {
    ...
    // Get collaborating engineers per project
    const [collabs] = await pool.query(`
      SELECT
        p.id AS project_id,
        p.project_name,
        p.engineer_id AS lead_engineer_id,
        ew.id AS collab_engineer_id,
        ew.name AS collab_engineer_name
      FROM projects p
    ...
    collabs.forEach(c => {
      ...
      collabMap[c.project_id].push({ id: c.collab_engineer_id, name: c.collab_engineer_name });
    });

    // Enrich engineer stats with collaboration data
    const enriched = engineers.map(e => {
      ...
      return {
        ...
        completed_projects: e.completed_projects + collabProjects.filter(c => {
          const proj = collabs.find(p => p.project_id === c.project_id);
